...
if (isset($_POST['submit'])) {
	if (!empty($_POST['check_list'])) {
		...
		foreach ($_POST['check_list'] as $selected) {
			...
			$qry_analisa = mysqli_query($conn, $sql_analisa) or die(mysqli_error());
			...
			if ($data_cek >= 1) {
				...
				$qry_tmp = mysqli_query($conn, $sql_tmp) or die(mysqli_error());
				while ($data_tmp = mysqli_fetch_array($qry_tmp)) {
					...
					$qry_rsakit = mysqli_query($conn, $sql_rsakit) or die(mysqli_error());
					while ($data_rsakit = mysqli_fetch_array($qry_rsakit)) {
						...
						mysqli_query($conn, $sql_input) or die(mysqli_error());
					}
				}
				// Gunakan Fungsi
				DelTmpAnlisa($u);
				...
				$qry_rgejala = mysqli_query($conn, $sql_rgejala) or die(mysqli_error());
				while ($data_rgejala = mysqli_fetch_array($qry_rgejala)) {
					...
					while ($data_rsakit = mysqli_fetch_array($qry_rsakit)) {
						// Data penyakit yang mungkin dimasukkan ke tmp
						$sql_input = mysqli_query($conn, "INSERT INTO tmp_penyakit (username,kode_penyakit) VALUES ('$u','$data_rsakit[kode_penyakit]')") or die(mysqli_error());
					}
				}
				// Menggunakan Fungsi
				AddTmpAnalisa($selected, $u);
	...
	$qry_cekh = mysqli_query($conn, $sql_cekh) or die(mysqli_error());
	$hsl_cekh = mysqli_num_rows($qry_cekh);
	$hsl_data = mysqli_fetch_array($qry_cekh);
	$cek_gejala_valid = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tmp_gejala where status='1'"));
	$cek_gejala_penyakit = mysqli_num_rows(mysqli_query($conn, "SELECT relasi_penyakit_gejala.* FROM relasi_penyakit_gejala,tmp_penyakit where relasi_penyakit_gejala.kode_penyakit=tmp_penyakit.kode_penyakit"));
	$hasilbobot = mysqli_query($conn, "SELECT bobot FROM relasi_penyakit_gejala, tmp_gejala WHERE kode_penyakit = '$hsl_data[kode_penyakit]' AND relasi_penyakit_gejala.kode_gejala = tmp_gejala.kode_gejala AND tmp_gejala.status =1") or die(mysqli_error());
	$persentase = 0;
...
*/
	if ($persentase == 0) {
		...
		$qry_pasien = mysqli_query($conn, $sql_pasien);
		...
		mysqli_query($conn, $sql_in) or die(mysqli_error());
